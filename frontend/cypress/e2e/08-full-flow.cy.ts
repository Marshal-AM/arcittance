...
/**
 * Full Arcittance dApp E2E Test Suite — Arc Testnet
 *
...
const PAYROLL_API  = `/api/payroll?vault=${Cypress.env("VAULT_ADDRESS")}`;
...
describe("2 · API Fetch & Dashboard Counts", () => {
  it("2.1  GET /api/payroll — valid shape and employee fields", () => {
    cy.request("GET", PAYROLL_API).then(({ status, body }) => {
...
describe("3 · Payroll Page", () => {
  ...
  before(() => {
    cy.request("GET", PAYROLL_API).then(({ body }) => { initialEmpCount = body.total; });
  });
