...
const payrollUrl = (vault?: string) => {
  const v = vault ?? Cypress.env("VAULT_ADDRESS");
  return `/api/payroll?vault=${v}`;
};

describe("API Routes", () => {
  it("GET /api/payroll returns correct shape", () => {
    cy.request("GET", payrollUrl()).then((response) => {
      expect(response.status).to.eq(200);
      expect(response.body).to.have.property("employees");
      expect(response.body).to.have.property("total");
      expect(response.body).to.have.property("vault");
  ...
  it("GET /api/payroll employee fields are strings (bigint serialised)", () => {
    cy.request("GET", payrollUrl()).then((response) => {
      if (response.body.employees.length > 0) {
        ...
        expect(emp.destinationChainId).to.be.a("number");
        expect(emp.destinationName).to.be.a("string");
        expect(emp.active).to.be.a("boolean");
      }
    });
  });

  it("GET /api/payroll employee destinationName is resolved", () => {
    cy.request("GET", payrollUrl()).then((response) => {
      response.body.employees.forEach((emp: { destinationName: string }) => {
        expect(emp.destinationName).to.not.be.empty;
        expect(emp.destinationName).to.not.include("undefined");
      });
    });
  });

  it("GET /api/payroll returns 400 without vault param", () => {
    cy.request({ url: "/api/payroll", failOnStatusCode: false }).then((response) => {
