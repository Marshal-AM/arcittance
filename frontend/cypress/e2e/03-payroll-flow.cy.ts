...
describe("Payroll Flow — UI state reflects on-chain changes", () => {
  ...
  const USDC  = Cypress.env("USDC_ADDRESS");
  const PK    = Cypress.env("DEPLOYER_PRIVATE_KEY");
  const RPC   = Cypress.env("ARC_RPC_URL");
  const payrollApi = `/api/payroll?vault=${VAULT}`;
  ...
  it("PayrollRoster shows employee count matching API", () => {
    cy.request("GET", payrollApi).then(({ body }) => {
      cy.visit("/payroll");
      if (body.total > 0) {
  ...
  it("Employee rows show destination chain name", () => {
    cy.request("GET", payrollApi).then(({ body }) => {
      if (body.employees.length > 0) {
        cy.visit("/payroll");
        const emp = body.employees[0];
        cy.get("table").contains(emp.destinationName).should("be.visible");
      }
    });
  });
  ...
  it("Connect wallet prompt is shown when not connected", () => {
    ...
    cy.contains("Connect your wallet to create").should("be.visible");
  });
