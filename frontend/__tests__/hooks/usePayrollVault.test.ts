...
describe("useRunPayroll", () => {
  it("uses gasLimit 2_000_000n and orchestrates CCTP after on-chain payroll", async () => {
    const mockWrite = jest.fn().mockResolvedValue(MOCK_TX_HASH);
    (useWriteContract as jest.Mock).mockReturnValue({
      writeContractAsync: mockWrite,
    });
    ...
    global.fetch = jest.fn().mockResolvedValue({
      ok:   true,
      json: async () => ({ payrollTxHash: MOCK_TX_HASH, cctpCompletions: [] }),
    }) as jest.Mock;
