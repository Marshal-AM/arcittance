...
const PRIMITIVES = [
  {
    label: "PAYROLL",
    title: "Recurring payroll vaults",
    desc: "Create an organisation vault, register employees with salary, interval, and destination chain. Run payroll in one keeper call — Arc-local transfer or CCTP cross-chain.",
    stats: [{ v: "USDC", l: "settlement" }, { v: "CCTP", l: "cross-chain" }],
    img: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/researcher-CvhqOuV6irGwBOnJoTGFlXdbyYBRjb.png",
  },
  {
    label: "ESCROW",
    title: "Milestone escrow",
    desc: "Lock funds at project start. Release on multi-party approval, or reclaim automatically when the deadline passes. No intermediary cut.",
    stats: [{ v: "N-of-M", l: "approvals" }, { v: "0%", l: "platform fee" }],
    img: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/coder-9bItvCegU6TXUqbX3tUXGBAtvkBkXp.png",
  },
  {
    label: "SUBSCRIBE",
    title: "Subscription billing",
    desc: "Create plans with charge amount and interval. Subscribers set an approved cap. Batch-charge in a single keeper run with on-chain enforcement.",
    stats: [{ v: "Cap", l: "enforced" }, { v: "Batch", l: "keeper" }],
  ...
  },
  {
    ...
    desc: "Fund via wallet or bank wire, convert USDC↔EURC with StableFX, deliver crypto or fiat. Transparent fees, live settlement, downloadable receipts.",
    stats: [{ v: "2", l: "funding paths" }, { v: "FX", l: "StableFX" }],
...
export function StackingAgentCards() {
  ...
  const [depth, setDepth] = useState<number[]>(PRIMITIVES.map(() => 0))

  useEffect(() => {
    function onScroll() {
      const nextDepth = PRIMITIVES.map((_, i) => {
